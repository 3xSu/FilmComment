...
public class CommentCleanupJob {
    ...
    public void autoCleanupExpiredComments() {
        ...
        try {
            ...
            if (!isLocked) {
                log.info("未获取到分布式锁，评论清理任务将由其他实例执行");
                return;
            }
    ...
    public void autoCleanupExpiredCommentsForTest() {
        String lockKey = "lock:job:comment:cleanup:test";
        RLock lock = redissonClient.getLock(lockKey);
        ...
        try {
            ...
            if (!isLocked) {
                log.info("测试环境：未获取到分布式锁，评论清理任务将由其他实例执行");
                return;
            }
            ...
            Thread.currentThread().interrupt();
            log.error("测试环境：评论清理任务获取分布式锁时被中断", e);
        } finally {
            if (isLocked && lock.isHeldByCurrentThread()) {
                lock.unlock();
                log.info("测试环境：评论清理任务分布式锁已释放");
            }
        }
    }
}